use crate::scaffolding::PlayerType;
...
impl RoomCode {
    ...
    /// 从整型值构造房间码
    fn from_value(mut value: u128) -> Self {
    ...
    fn start_room_host(&self, port: u16, player: Option<&str>, public_servers: Vec<String>) {
...
mod tests {
    ...
    fn test_room_code_generate() {
        ...
        dbg!("{}", &room_code.code);
        assert!(RoomCode::parse(&room_code.code).is_some());
    }
    ...
    fn test_room_code_is_valid() {
        ...
        assert!(RoomCode::parse("U/LX2M-2A87-YXMZ-2HJJ").is_some());
        assert!(RoomCode::parse("U/YS3D-LTH4-6AUC-MBFB").is_some());
        assert!(RoomCode::parse("U/BC4D-A51Z-ZE3P-LAP9").is_some());
        assert!(RoomCode::parse("U/UCAW-UY61-QD93-UL9X").is_some());
        assert!(RoomCode::parse("U/KP8W-WH0Y-NQCY-MXJ4").is_some());
    }
}
